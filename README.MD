...
instapack also has a custom-made but standard-compliant Sass module system using `@import` syntax:
...
- Include files, resolved from `node_modules`. Reads `package.json` to locate the file in `sass` or `style` field.
