...
instapack is battle-tested :hocho: and is designed to cover *most normal use cases* when developing a modern web app. Powered by [webpack](https://webpack.js.org), instapack effortlessly consumes modern JS modules (ES, CommonJS, UMD) and more (plain HTML templates, Vue SFC, TypeScript JSX).
...
- `--https` augments `--serve` flag, allowing the Hot Reload dev server to use `https://` protocol.
...
- `--experimental-react-refresh` enables dev server with [React Fast Refresh](https://facebook.github.io/react-native/blog/2019/09/18/version-0.61) (new native Hot Reload).

- `--no-sourcemaps` disables source maps, producing undebuggable outputs. :bug: (Slightly improves build speed)

- `--stats` generates `stats.json` next to the TypeScript build outputs, which can be analyzed by a third-party tool: [webpack-bundle-analyzer](https://www.npmjs.com/package/webpack-bundle-analyzer). For sanity, this flag will be ignored when using `-d` or `-w` flags.
...
- `copy` allows copying static assets from the npm folder (`node_modules`) into the project output folder (e.g. `wwwroot`) during build.
  ...
  - **This is a PREVIEW feature** inspired by [Visual Studio LibMan](https://docs.microsoft.com/en-us/aspnet/core/client-side/libman/libman-vs?view=aspnetcore-3.0).
...
- `transpileLibraries` accepts an array of strings of package names, forcing transpilation of `.js` files bundled from those packages in the `node_modules` folder using the TypeScript compiler, following the `target` compiler option in `tsconfig.json`
...
- instapack will not resolve symlinks if TypeScript compiler option `preserveSymlinks` is set to `true`
...
instapack 8.0.0 supports [ESLint configuration](https://eslint.org/docs/user-guide/configuring), applied directly in-memory to TypeScript source code during type-check phase. 
...
# or...
yarn add eslint typescript @typescript-eslint/eslint-plugin @typescript-eslint/parser -D
```
...
**However, these lint errors will not be visible in Visual Studio Code...** To remedy this issue, add two more packages to the project and use the following special ESLint configuration instead:
...
instapack supports [`.babelrc`](https://babeljs.io/docs/usage/babelrc/) in the project root folder. **Babel transformations will be applied AFTER TypeScript compilation.** Here are some use cases where this feature can be useful:

- Babel may be used instead of TypeScript to transpile JS to ES5: set TypeScript `target` to `esnext` then use [`@babel/preset-env`](https://github.com/babel/babel/tree/master/packages/babel-preset-env)

- Babel may be used instead of TypeScript to compile JSX: set TypeScript `jsx` to `preserve` then use the framework-compatible JSX plugin. For example: `@babel/preset-react`, `babel-plugin-transform-vue-jsx`, `babel-plugin-inferno`
...
- Create script injections in place of the usual JS output files, directed to respective scripts on the build server.
...
[01:24:28] Inject <script> D:/VS/project/wwwroot/js/ipack.dll.js --> http://localhost:28080/ipack.dll.js
[01:24:28] Inject <script> D:/VS/project/wwwroot/js/ipack.js --> http://localhost:28080/ipack.js
```
...
> If not using React Refresh, use [react-hot-loader](https://github.com/gaearon/react-hot-loader) package and manually configure the project using instructions provided.
...
- Basic CSS support in `<style>` (or `<style scoped>` or `<style module>`) code blocks should work, but Sass cannot be used and the resulting CSS will NOT be auto-prefixed and minified. 
...
- [According to the official Sass CSS Imports specification](https://github.com/sass/sass/blob/master/accepted/css-imports.md#handling-an-import-rule), these imports will be treated as *'plain CSS'* `@import` rules (file not included in resulting bundle):

  - The imported URL / query path begins with `http://` or `https://`

  - **The query path explicitly ends with `.css` extension.** :warning: 

    - NOT included: `@import './thing.css';`

    - Included: `@import './thing';`
...
- Includes `.scss` and `.css` files relative to the source. ([Standard Sass behavior](https://sass-lang.com/documentation/at-rules/use#load-paths))
...
**If it worked when using standard JS, it WILL work with instapack.** (Other frameworks not shipped in new project templates such as Angular 2+, Preact, Inferno.js, Mithril.js are known to be working with instapack.)
...
COPY --from=build /src/MyApp/bin/Release/netcoreapp3.0/publish /app
WORKDIR /app
...
Windows Defender or other anti-virus software apparently slow down package restores and IDEs when opening projects.
