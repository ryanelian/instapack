...
> **CAUTION:** `cordova platform add` and `cordova plugin add` (or remove) will destroy your `node_modules`. Delete `package-lock.json` and `yarn.lock` then re-run `npm` or `yarn` after invoking those commands.
